...
run_all_tests() {
    ...
    echo -e "\n${YELLOW}--- Building all tests ---${NC}"
    if ! make -C "." all; then
        echo -e "${RED}Build failed for tests${NC}"
        status=1
    fi

    echo -e "\n${YELLOW}--- Running assembler unit tests ---${NC}"
    if ! ./test_assembler; then
        echo -e "${RED}[FAIL] Assembler unit tests${NC}"
        status=1
    else
        echo -e "${GREEN}[PASS] Assembler unit tests${NC}"
    fi

    echo -e "\n${YELLOW}--- Running emulator unit tests ---${NC}"
    if ! ./test_emulator; then
        echo -e "${RED}[FAIL] Emulator unit tests${NC}"
        status=1
    else
        echo -e "${GREEN}[PASS] Emulator unit tests${NC}"
    fi
...
# Main execution
main() {
    echo -e "${YELLOW}=== RISC-V Complete Test Suite ===${NC}"
    ...
    # Run all tests
    test_status=0
    run_all_tests || test_status=1

    # Final cleanup (keep binaries)
    echo -e "\n${YELLOW}--- Final cleanup (keeping binaries) ---${NC}"
    find . -name "*.o" -delete 2>/dev/null || true
    rm -f test_assembler test_emulator test_integration 2>/dev/null || true
    rm -f *.bin *.s 2>/dev/null || true
